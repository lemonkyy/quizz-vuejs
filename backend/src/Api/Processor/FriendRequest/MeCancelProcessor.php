...
class MeCancelProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): JsonResponse
    {
        ...
        if (!$friendRequest || $friendRequest->getSender() !== $user) {
            return new JsonResponse(['code' => 'ERR_FRIEND_REQUEST_NOT_FOUND', 'error' => 'Friend request not found or not allowed'], 404);
        }
        ...
        if ($friendRequest->getRevokedAt() !== null) {
            return new JsonResponse(['code' => 'ERR_FRIEND_REQUEST_ALREADY_REVOKED', 'error' => 'Friend request already revoked'], 400);
        }

        $friendRequest->setRevokedAt(new \DateTimeImmutable());
        $this->entityManager->flush();

        return new JsonResponse(['code' => 'SUCCESS', 'message' => 'Friend request revoked'], 200);
    }
}